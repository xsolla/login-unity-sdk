...
public class UserProfilePageManager : MonoBehaviour
{
	...
	private void UpdateCommonEntries(UserProfileEntryType entryType, string newValue)
	{
		...
		var token = DemoController.Instance.GetImplementation().Token;
		DemoController.Instance.GetImplementation().UpdateUserInfo(token, infoUpdatePack,
			onSuccess: newInfo =>
			{
	...
	private void UpdateUserPhoneNumber(string newValue)
	{
		...
		DemoController.Instance.GetImplementation().ChangeUserPhoneNumber(token, newValue,
	...
	private IEnumerator DeleteUserPhoneNumber()
	{
		...
		if (userInfo != null)
		{
			...
			DemoController.Instance.GetImplementation().DeleteUserPhoneNumber(token, oldPhoneNumber,
				onSuccess: () => _isProfileUpdated = true,
				onError: _commonErrorCallback);
		}
	}
