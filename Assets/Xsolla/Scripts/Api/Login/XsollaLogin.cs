...
namespace Xsolla.Login
{
	...
	public partial class XsollaLogin : MonoSingleton<XsollaLogin>
	{
		...
		private string AnalyticUrlAddition => $"engine=unity&engine_v={Application.unityVersion}&sdk=login&sdk_v={Constants.LOGIN_SDK_VERSION}";
			...
			get
			{
				if (Application.platform != RuntimePlatform.WebGLPlayer)
				{
					return new List<WebRequestHeader>
					{
						...
						new WebRequestHeader() { Name = "X-SDK-V", Value = Constants.LOGIN_SDK_VERSION.ToUpper() },
					};
				}
				else
					return new List<WebRequestHeader>();
